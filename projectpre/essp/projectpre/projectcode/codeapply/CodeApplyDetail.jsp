...
<%
  ...
  actionSrc_masterData=contextPath+"/project/apply/previewProjectAcntApp.do";
  actionSrc_technicalData=contextPath+"/projectpre/projectcode/codeapply/TechnicalData.jsp";
  ...
  <head>
     ...
     <script type="text/javascript" language="javascript">
       ...
       function onTabChange(){
         try {
            ...
            //if(getTabIndex(tabDetail)==1){
               masterData.disableBtn("add", "<%=request.getParameter("isFinance")%>");
            //}
            //if(getTabIndex(tabDetail)==2){
           
            //  technicalData.onLoadBody();
            //}
            //if(getTabIndex(tabDetail)==3){
           //    projectData.onLoadBody();
           // }
            
            applyRecordList.disableBtn();
       ...
       //invokeSingleMethod("onAddData",'',tabMap);
        applyRecordList.onAddData("<%=request.getParameter("isFinance")%>");
     
         
       }
       
       function onDeleteData(){
         applyRecordList.onDeleteData()
       }
       ...
       function onRefreshData(rid){
        ...
        if(rid==""||rid==null){
          ...
          masterData.location="<%=contextPath%>/project/apply/previewProjectAcntApp.do?rid="+rid+"&accessType=edit&appType=add&isFinance="+isFinance;
          technicalData.location="<%=contextPath%>/project/apply/previewTechInfoApp.do?rid="+rid+"&accessType=edit";
          projectData.location="<%=contextPath%>/project/apply/previewCustomerInfoApp.do?rid="+rid+"&accessType=edit";
    ...
    <table cellspacing="0" cellpadding="1" height="100%" width="100%">
       <tr height="28%" >
		 <td class="wind">
		    <iframe 
				id="applyRecordList" 
				src="<%=contextPath%>/project/apply/listAllProjectAcntApp.do?appType=add"
				scrolling="yes" 
