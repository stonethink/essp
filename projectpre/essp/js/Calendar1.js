var weekend = [0,6];
var todayColor = "#cccccc";
var weekendColor = "#ffffcc";
var fontface = "Verdana";
var fontsize = 10;			// in "pt" units; used with "font-size" style element
...
function Calendar(p_item, p_month, p_year, p_format) {
	...
	if (p_month == null) {
		...
		//this.gMonth = (this.gMonth.toString().length < 2) ? "0" + this.gMonth : this.gMonth;
		this.gYearly = false;
	}
...
Calendar.prototype.getMonthlyCalendarCode = function() {
	...
	vCode += ("<div align=center><TABLE width=174 BORDER=0 BGCOLOR=\"" + this.gBGColor + "\" style='font-size:" + fontsize + "pt;'>");
...
Calendar.prototype.show = function() {
	...
	var prev_MM = this.gMonth;
	var prev_YYYY = parseInt(this.gYear)-1;
	
	
	var next_MM = this.gMonth;
	var next_YYYY = parseInt(this.gYear)+1;
	...
	var vDay=1;
	var kwh;
	ggWinContent += ("<TABLE WIDTH='100%' BORDER=0 CELLSPACING=0 CELLPADDING=0 BGCOLOR='#94aad6' style='font-size:" + fontsize + "pt;'><TR><TD ALIGN=center>");
	
	ggWinContent += ("<A HREF=\"javascript:void(0);\" " +
		"onMouseOver=\"window.status='Go back one year'; return true;\" " +
		"onMouseOut=\"window.status=''; return true;\" " +
		"onmousedown=\"show_detail_last('" +
					(parseInt(this.gYear)-1) +""+  (parseInt(this.gMonth)+1) +
					"'); ChgSTypeD.submit(); return true;\" " +
		"onClick=\"Build(" + 
		"'" + this.gReturnItem + "', '" + prev_MM + "', '" + prev_YYYY + "', '" + this.gFormat + "'" +
		");" +
	...
	ggWinContent += "       </TD><TD ALIGN=center>" +
		...
		"onMouseOver=\"window.status='set date to " + this.gMonthName + "'; return true;\" " +
		"<B><FONT color=white size=2><B>" + 
		this.gYear + "- " + this.gMonthName +
		"<B></font></A>" + 
		"</TD><TD ALIGN=center>";
	...
	ggWinContent += ("<A HREF=\"javascript:void(0);\" " +
		...
		"onmousedown=\"show_detail_next('" +
					...
					"'); ChgSTypeD.submit(); return true;\" " +
		"onClick=\"Build(" + 
		"'" + this.gReturnItem + "', '" + next_MM + "', '" + next_YYYY + "', '" + this.gFormat + "'" +
		");" +
		"\">>><\/A></TD><TD ALIGN=center>");	
...
Calendar.prototype.showY = function() {
	...
	ggWinContent += ("<TABLE WIDTH='100%' BORDER=0 CELLSPACING=0 CELLPADDING=0 BGCOLOR='#aee6bf' style='font-size:" + fontsize + "pt;'><TR><TD ALIGN=center>");
	ggWinContent += ("[<A HREF=\"javascript:void(0);\" " +
		"onMouseOver=\"window.status='Go back one year'; return true;\" " +
		"onMouseOut=\"window.status=''; return true;\" " +
		"onClick=\"Build(" + 
...
Calendar.prototype.cal_header = function() {
	...
	vCode = vCode + "<TD WIDTH='14%'><FONT FACE='" + fontface + "' COLOR=red SIZE=1>SUN</FONT></TD>";
	vCode = vCode + "<TD WIDTH='14%'><FONT FACE='" + fontface + "' COLOR='" + this.gHeaderColor + "' SIZE=1>MON</FONT></TD>";
	vCode = vCode + "<TD WIDTH='14%'><FONT FACE='" + fontface + "' COLOR='" + this.gHeaderColor + "' SIZE=1>TUE</FONT></TD>";
	vCode = vCode + "<TD WIDTH='14%'><FONT FACE='" + fontface + "' COLOR='" + this.gHeaderColor + "' SIZE=1>WED</FONT></TD>";
	vCode = vCode + "<TD WIDTH='14%'><FONT FACE='" + fontface + "' COLOR='" + this.gHeaderColor + "' SIZE=1>THU</FONT></TD>";
	vCode = vCode + "<TD WIDTH='14%'><FONT FACE='" + fontface + "' COLOR='" + this.gHeaderColor + "' SIZE=1>FRI</FONT></TD>";
	vCode = vCode + "<TD WIDTH='14%'><FONT FACE='" + fontface + "' COLOR=red SIZE=1>SAT</FONT></TD>";
	
	vCode = vCode + "</TR><table cellPadding=1 border=0 cellSpacing=1 bgColor=silver bordercolor=silver><td><TABLE width=174 bgColor=white cellSpacing=1 cellPadding=1 border=0>";
...
Calendar.prototype.cal_data = function() {
	...
	var vOnLastDay=0;
	var vCode = "";
	var vDate_yy = vDate.getYear();
	var vDate_mm = vDate.getMonth();
	var vDate_ww = vDate.getDay();
	//var vDate_all =  vDate_yy +""+ vDate_mm +""+ vDate_ww;
	var vDate_all =  vDate_ww;
	/*
	...
	*/
	...
	for (i=0; i<vFirstDay; i++) {
		vCode = vCode + "<TD WIDTH='12%'" + this.write_weekend_string(i) + "><FONT FACE='" + fontface + "'> </FONT></TD>";
	}

	// Write rest of the 1st week
	for (j=vFirstDay; j<7; j++) {
		vCode = vCode + "<TD WIDTH='12%'" + this.write_weekend_string(j) + "><FONT FACE='" + fontface + "'>" + 
			"<A HREF='#' " + 
	...
	for (k=2; k<7; k++) {
		...
		for (j=0; j<7; j++) {
			...
			vCode = vCode + "<TD WIDTH='12%'" + this.write_weekend_string(j ,vDay)  + "><FONT FACE='" + fontface + "'>" + 
				"<A HREF='#' " +
					"onMouseOver=\"window.status='set date to " + this.format_data(vDay) + "'; return true;\" " +
					"onMouseOut=\"window.status=' '; return true;\" " +
					"onclick=\"show_detail('" +
...
//	for (m=1; m<(7-j); m++) {
//		if (this.gYearly)
//			vCode = vCode + "<TD WIDTH='12%'" + this.write_weekend_string(j+m) + 
...
//	}
	...
	var vNowMonth = today.getMonth() + 1;
	var vNowYear = today.getFullYear();
	var vmyDay = ""+ vNowYear +""+ vNowMonth +""+ vNowDay;
	//var vmyDay = vNowDay;
	//alert("vmyDay is " + this.format_data(vmyDay));
	
	vCode = vCode + "<TABLE><TR><TD WIDTH='90%';ALIGN=LEFT ><FONT size=2 FACE='" + fontface + "'>" + 
				"<A HREF='javascript:void(0);' " +
				"onMouseOver=\"window.status='set date to " + this.format_data(today) + "'; return true;\" " +
				"onMouseOut=\"window.status=' '; return true;\" " +
				"onmousedown=\"show_detail_today();  ChgSTypeD.submit(); return true;\" " +
				//"onClick=\"show_detail_today('" + 
				//this.format_data(vmyDay) + 	 
				//"');\" >" + 
				">" + 
				"Today"+
				"</A>" + 
...
Calendar.prototype.write_weekend_string = function(vday1,vday) {
	var i;
	var vNowDay = gNow.getDate();
	
	// Return special formatting for the weekend and today.
	for (i=0; i<weekend.length; i++) {
		if ( vday ==  tempday){
			
		 	return (" BGCOLOR=\"" + todayColor + "\"");
		 }else if (vday == vNowDay){
			return (" BGCOLOR=\"" + todayColor + "\"");
			alert("tempday",tempday);
		 }else if (vday1 == weekend[i])
...
function show_calendar() {
	...
	*/
	...
	if (arguments[1] == null)
		...
		//p_month = new String( getdate_mm - 1 );
	else
		p_month = arguments[1];
	if (arguments[2] == "" || arguments[2] == null)
		p_year = new String(gNow.getFullYear().toString());
		//p_year = new String( getdate_ty );
	else
		p_year = arguments[2];
	if (arguments[3] == null)
		p_format = "YYYY-MM-DD";
...
if(typeof ol_close=='undefined'){var ol_close="ÊÙë·";}
...
if(ocss==CSSSTYLE)txt="<TABLE WIDTH=174 BORDER=0 CELLPADDING=0 CELLSPACING=2 style=\"background-color: #94aad6;height: "+oheight+oheightunit+";\"><TR><TD><TABLE WIDTH=100% BORDER=0 CELLPADDING=0 CELLSPACING=0 style=\"color: "+ofgcolor+";background-color: white;height: "+oheight+oheightunit+";\"><TR><TD VALIGN=TOP><FONT style=\"font-family: "+otextfont+";color: "+otextcolor+";font-size: "+otextsize+otextsizeunit+";text-decoration: "+otextdecoration+";font-weight: "+otextweight+";font-style:"+otextstyle+"\">"+text+"</FONT></TD></TR></TABLE></TD></TR></TABLE>";
