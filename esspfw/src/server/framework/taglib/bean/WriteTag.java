 ...
 *
 * @version $Rev: 54929 $ $Date: 2006/08/15 10:55:06 $
 */
public class WriteTag extends TagSupport {
    server.framework.taglib.util.TagUtils fwUtil=new server.framework.taglib.util.TagUtils();
    /**
    ...
    public int doStartTag() throws JspException {
        ...
        if (ignore) {
            if (fwUtil.lookup(pageContext, name, scope) == null) {
                return (SKIP_BODY); // Nothing to output
            }
        }
        // Look up the requested property value
        Object value = fwUtil.lookup(pageContext, name,
                property, scope);
        if (value == null) {
            value="";
        }
