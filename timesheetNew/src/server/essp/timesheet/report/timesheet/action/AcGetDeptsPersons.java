 ...
 * @version 1.0
 */
public class AcGetDeptsPersons extends AbstractESSPAction {
    /**
     * executeAct
     *
    ...
    public void executeAct(HttpServletRequest httpServletRequest,
        ...
        if(!isPMO) {
            ...
            data.getReturnInfo().setReturnObj(DtoTsDetailReport.DTO_PERSON_LIST,
                                              result.get(DtoTsDetailReport.DTO_PERSON_LIST));
        	...
        	data.getReturnInfo().setReturnObj(DtoTsDetailReport.DTO_DEPT_LIST, depts);
        	Vector persons = new Vector();
        	DtoComboItem item = null;
        	if(depts != null && depts.size() > 0) {
        		item = (DtoComboItem) depts.get(0);
        		persons = service.getPersonsForPmo((String)item.getItemValue());
        	}
        	data.getReturnInfo().setReturnObj(DtoTsDetailReport.DTO_PERSON_LIST, persons);
        }
