...
<html>
	<head>
		...
		try{  
	  		...
	  		window.location="<%=contextPath%>/timesheet/syncexception/carryForward.do?rid="+rid;
	  	}catch(e){}
	    }
	    
	    function onCancelData() {
	    try{
	    	if(confirm('<bean:message bundle="timesheet" key="timesheet.syncexception.confirmCancel"/>')){
	    		var rid = currentRow.selfproperty;
	    		window.location="<%=contextPath%>/timesheet/syncexception/cancelException.do?rid="+rid;
	    	}
	    }catch(e){}
	    }
	   ...
	   //当进入此页时，自动选中第一行，并国际化表头
		...
		try{
	        ...
	        var str0="<bean:message bundle="timesheet" key="timesheet.syncexception.model"/>";		
			var str1="<bean:message bundle="timesheet" key="timesheet.syncexception.operation"/>";
			var str2="<bean:message bundle="timesheet" key="timesheet.syncexception.message"/>";
			var str3="<bean:message bundle="timesheet" key="timesheet.syncexception.status"/>";
			var str4="<bean:message bundle="timesheet" key="timesheet.syncexception.rct"/>";			
			...
			cells[1].innerHTML=cells[1].innerHTML.replace("Operation",str1);
			cells[1].title=cells[1].title.replace("Operation",str1);
			cells[2].innerHTML=cells[2].innerHTML.replace("Content",str2);
			cells[2].title=cells[2].title.replace("Content",str2);
			cells[3].innerHTML=cells[3].innerHTML.replace("Status",str3);
			cells[3].title=cells[3].title.replace("Status",str3);
			cells[4].innerHTML=cells[4].innerHTML.replace("Rct",str4);
			cells[4].title=cells[4].title.replace("Rct",str4);
			 var firstRow=table.rows[thead_length];
		       disableBtn();
			   rowOnClick(firstRow);
	...
	<body>
		...
		<ec:table tableId="exceptionList" items="webVo" var="exception"
			scope="request"
			action="${pageContext.request.contextPath}/timesheet/syncexception/listException.do"
			...
			<ec:row onclick="rowOnClick(this);" selfproperty="${exception.rid}">
				...
				<ec:column style="width:20%" property="errorMessage" tooltip="${exception.errorMessage}" title="Content" />
