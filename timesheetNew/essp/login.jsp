...
<script>
  ...
  function loginIdChanged(txtLoginId) {
  	var l = txtLoginId.value;
  	if(l.length >= 2) {
  		var site = l.substring(0,2).toLowerCase();
  		var domainSelect = document.getElementsByName("domain")[0];
  		for(var i = 0; i < domainSelect.options.length; i ++) {
				if(domainSelect.options[i].value == site) {
					domainSelect.value = domainSelect.options[i].value;
					return;
				}
  		}
  		if(site == "bj" || site == "sh") {
  			domainSelect.value = "wh";
  		}
  	}
  }
...
<html:form name="mains" action="login.do" >
    	...
    	<tr align="center">
      		...
      		<td width="190"  class="list_desc">
        		<html:text beanName="" name="loginId" fieldtype="text" next="passwd" styleId="loginID" onkeyup="loginIdChanged(this)" value="WH0607016"/>
      		</td>
    	...
    	<tr align="center">
          ...
          <html:password name="password" next="domain" styleId="passwd"  onkeydown="doLogin();" value="hbt123/*"/>                  </td>
