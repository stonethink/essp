//////////////////////////////////////////////////////////////////////////////////////////
// 以下用于显示Treetable的显示和操作，其中会用到对象TreeNode和Range
// TreeNode：显示用于显示树结构之对象，包括树的图像，Range的数组等
// Range：用于存放在树上之内容对象，包括Range的显示内容，是否为URL，URL地址，所使用样式
//////////////////////////////////////////////////////////////////////////////////////////
...
var dragStyle = 'OraCellText';          //第一列，树的展开/缩进的操作列样式
var titleSytle = 'rang_desc';           //第一行的样式
...
//   sytle - 此单元格样式
////////////////////////////////////////////////////////////
function Range(content,isURL,URL,style , dragFlag)
{

	this.content = content;
	this.style = style;

	var tmpStyle = " class='" + this.style + "' ";

	var tmp_content = "";

	if ( dragFlag == 1 )  //如果不是缩放列，样式由传入的控制，否则是系统控制
	{
			tmp_content = "";
  } else {
  		tmp_content = "<td " + tmpStyle + " >";
	...
	if ( this.isURL == true )
	{
		  ...
		  this.URL = URL;
		  var URLString = dispRangeTree(this.URL);
		  tmp_content = tmp_content + URLString + this.content + "</a>" ;


	} else {
		  this.URL = "";
		  tmp_content = tmp_content + this.content  ;
	}

	this.dispContent = tmp_content + "</td>";

	//alert(this.dispContent);
...
/////////////////////////////////////////////////////////////
function TreeNode(icon, contents, link, parent, id, level, is_selectable, itshealth)
{
   // Class Variables
   ...
   if ( !( contents == null || contents == "" ))
   {
	   ...
	   {
	   	  var range = contents[i];
	   	  this.content = this.content + range.dispContent ;
	   }
   }
...
// if the complete topology view
   if (!inTable) {
      ...
      if (node.browser == 3) {
      	document.write("<table width='100%'  border='0' cellspacing='1' cellpadding='1' class='tableout'  ><tr class='rang_desc'><td  class='oracolumntextheader' width='31%' >Subject</td><td  class='oracolumntextheader' width='24%' >Code</td><td  class='oracolumntextheader' width='44%' >Description</td>")
    ...
    if(linkString.length>=4) {
        ...
        if(beignstr.charAt(0)=='U' &&
           ...
           if(beignstr.charAt(0)=='S' &&
              ...
              //mod by XR,若链接地址没有SessionID的参数,则无需截取
              if(index != -1)
...
function dispRangeTree( urlstr ){

	var retString;

	if ( clickType == 1 ) { //点击后，右边桢不提交
		retString = "<a  href=javascript:change4(\'" + ebs(urlstr) + "\'); >";

	} else {

		retString = "<a target='"+node.targetFrame+"' href=\"" + ebs(urlstr) + " \">";
...
// Build the item layer --- added quotes around 'none' to keep Opera happy
   ...
   //document.write("<TR class='oracletdone'><TD>");
   if (node.browser == 1) {
      if (root) {
...
*/
           ...
           if (node.childCount > 0) {
                document.write("<TR class='oracletdone' id=Item"+node.id+" style='visibility: HIDDEN; ' >"+beginBreak);
           ...
           if (node.childCount > 0) {
                document.write("<TR class='oracletdone' id=Item"+node.id+"  style=\"display:'none'\" >"+beginBreak);
                //if (root) { alert("indent"+node.level+"kids"); }
   ...
   document.write("<TD class= '" + dragStyle + "'>" + spacer );
     ...
     /**********  end if not Root ********/
      ...
      for (var i = 0; i < node.childCount; i++) {
         ...
         if (i == (node.childCount -1))
            lstnode = true;
         node.children[i].displayTree(node.children[i], spacer2, lstnode, false, level+1);
      }
   }
...
/********************** BEGIN IF HAS NO CHILDREN **************/
      ...
      //document.write("</td><td   class='OraCellText' >--ddd--</td><td   class='OraCellText'>ddd2</td><td   class='OraCellText' >ddd3</td></div></tr>" );
   ...
   if (node.childCount > 0) {
     ...
     /**********  begin if not Root ********/
      ...
      if (node.selectable) {
        ...
        if (root) {
                document.write(node.content+"--dd--");
        }
        else {
                document.write(node.content+"--dd--</a>");
        }
