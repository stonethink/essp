...
<html>
    ...
    function onAddData(){
	   ...
	   var url='<%=contextPath%>/customer/add/initialCustApp.do';
	   var windowTitle="";
     ...
     var status=currentRow.cells[4].innerText; 
      if(currentRow==null) {
       ...
       if(confirm("<bean:message bundle="application" key="global.confirm.delete"/>")) {
    	window.location="<%=contextPath%>/customer/add/deleteCustApp.do?CODE="+currentRow.selfproperty;
   ...
   function rowOnClick(rowObj) {
	...
	function autoClickFirstRow(){
	  try{
	    var table=document.getElementById("customerCodeList_table");
		...
		var str2="<bean:message bundle="projectpre" key="customer.ClientShortName"/>";
		...
		cells[2].innerHTML=cells[2].innerHTML.replace("customer Short",str2);
		cells[3].innerHTML=cells[3].innerHTML.replace("Group Cust No",str3);
		...
		cells[2].title=cells[2].title.replace("customer Short",str2);
		cells[3].title=cells[3].title.replace("Group Cust No",str3);	
  ...
  %>
    <ec:table 
       tableId="customerCodeList"
       items="webVo"
       var="aCode" scope="request" 
       action="${pageContext.request.contextPath}/customer/add/listAllCustAppInfo.do"  
       ...
       >
        ...
        <ec:row onclick="rowOnClick(this);" selfproperty="${aCode.rid}">             
             <ec:column width="15%" property="rid" tooltip="${aCode.rid}" title="Apply No" />
             <ec:column width="15%" property="regId" tooltip="${aCode.regId}" title="Register No" />
             <ec:column width="20%" property="customerShort" tooltip="${aCode.regId}" title="customer Short" />
             <ec:column width="15%" property="groupId" tooltip="${aCode.groupId}" title="Group Cust No" />          
             <ec:column width="20%" property="applicationDate" tooltip="${aCode.applicationDate}" title="Apply Date" />
             <ec:column width="15%" property="applicationStatus" tooltip="${aCode.applicationStatus}" title="Status Code"/>
         </ec:row>      
     </ec:table>
