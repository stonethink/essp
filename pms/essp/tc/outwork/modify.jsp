...
<%@page import="db.essp.tc.TcOutWorker"%>
<%@page import="com.wits.util.comDate"%>
<%@page import="itf.hr.IHrUtil"%>
<%@page import="itf.hr.HrFactory"%>
<%@page import="itf.account.AccountFactory"%>
<%
  ...
  if (request.getAttribute("AOutWorker") != null) {
    TcOutWorker worker = (TcOutWorker) request.getAttribute("AOutWorker");

//    IHrUtil hrUtil=HrFactory.create();
//    DtoUser workerDetail=hrUtil.findResouce(worker.getLoginId());
    orgId=AccountFactory.createInfoUtil().getAccountByRID(worker.getAcntRid()).getOrganization();
    ...
    isAuto = worker.getIsAutoWeeklyReport().booleanValue();
  }
...
function sumbitUpdate(){
  ...
  var update='<%=request.getContextPath()%>/tc/outwork/outWorkerUpdate.do?rid='+<%=rid%>+'&acntRid='+accountId+'&loginId='+loginId+'&activityRid='+activityId;
//  alert(update);
...
<center class="form_title">Update Travels On Office Business Info</center>
<center>
  <html:form id="addForm" action="" method="POST">
    <table width="650" border="0" cellspacing="0" cellpadding="0">
      <tr>
        ...
        <td width="200">
          <ws:select id="depart" property="depart_code" type="server.essp.tc.outwork.logic.DepartmentSelectImpl" onchange="depart_codeSelectChangeEventaccount('null');" orgIds="<%=orgId%>" style="width:200px;background-color:#FFFACD" disabled="true"/>
        </td>
        <td width="50">&nbsp;</td>
        <td class="list_range">Account</td>
        <td width="200">
          <ws:upselect id="account" property="account_id" up="depart_code" type="server.essp.tc.outwork.logic.AccountSelectImpl" onchange="account_idSelectChangeEventemployee('null');account_idSelectChangeEventactivity('null');" orgIds="<%=orgId%>" style="width:200px;background-color:#FFFACD" default="<%=acntRid%>"/>
        </td>
      </tr>
      <tr>
        <td height="30" class="list_range">Employee</td>
        <td width="200">
          <ws:upselect id="employee" property="employee_id" up="account_id" type="server.essp.tc.outwork.logic.EmployeeSelectImpl" orgIds="<%=orgId%>" default="<%=emp%>" style="width:200px;background-color:#FFFACD"/>
        </td>
        <td>&nbsp;</td>
        <td class="list_range">Activity</td>
        <td width="200">
          <ws:upselect id="activity" property="activity_id" up="account_id" type="server.essp.tc.outwork.logic.ActivitySelectImpl" orgIds="<%=orgId%>" default="<%=activityRid%>" style="width:200px"/>
        </td>
      </tr>
      <tr>
      ...
      <tr>
        ...
        <td width="200">
          <html:text name="days" beanName="" fieldtype="text" styleId="input_date" onclick="calculateDays();" defaultvalue="true" value="<%=days%>" req="true"/>
        </td>
        <td>&nbsp;</td>
      ...
      <tr>
        ...
        <td width="200">
          ...
          <%} else {          %>
            <option value="true" selected>Auto Fill</option>
            <option value="false">Not Auto Fill</option>
          <%}          %>
          </select>
        </td>
        <td>&nbsp;</td>
        ...
        <td width="200">
          <html:select name="evectionType" styleId="select_date" req="true" value="<%=evectionType%>">
    ...
    <input type="button" class="button" name="Update" value="update" onClick="sumbitUpdate();">
    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
