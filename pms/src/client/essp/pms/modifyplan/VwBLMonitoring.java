...
import client.essp.pms.modifyplan.popselect.MonitorWbsActivityDrogtarget;
...
public class VwBLMonitoring extends VWGeneralWorkArea implements
...
/////// 段1，定义界面：定义界面（定义控件，设置控件名称，光标控制顺序）、定义界面事件
    VwBLMonitoringList blMonitorList;
    ...
    private void jbInit() throws Exception {
        blMonitorList = new VwBLMonitoringList(this.filterLbl, accountLabel);
        project = new GanttProject(false, true);
        planArea.setDividerLocation(650);
        ...
        planArea.setTopComponent(blMonitorList);
...
//        planArea.setAutoscrolls(false);
        ...
        new MonitorWbsActivityDrogtarget(blMonitorList,this).createDrogTarget();
...
//        tdArea.getSplitPane().setDividerLocation(upMaxHeight);
    ...
    private void resetTreeHeight() {
        int rows = 1;
        if (leftTree != null) {
            ITreeNode root = (ITreeNode) blMonitorList.getModel().getRoot();
    ...
    public void refreshGraphic() {
        taskIdCounter = 1;
        VMTreeTableModelForBL model = (VMTreeTableModelForBL) blMonitorList.
                                      getModel();
    ...
    public void buildRlation() {
        List relationList = blMonitorList.loadAllRelation();
    ...
    public Task getTaskByRid(Long rid) {
        DtoWbsActivity dto = getActivityCodeByRid((ITreeNode) blMonitorList.
                                                  getModel().
                                                  getRoot(), rid);
        if (dto != null) {
    ...
    public void setLeftBtnVisible(boolean flag) {
        ...
        blMonitorList.btnWorker.setVisible(flag);
    ...
    private void addUICEvent() {
        blMonitorList.getTreeTable().getTreeTableModel().addTreeModelListener(new
            TreeModelListener() {
            public void treeNodesChanged(TreeModelEvent e) {
//                System.out.println("数据改变，进入其监听");
                DtoWbsActivity dataBean = (DtoWbsActivity) blMonitorList.
...
//                System.out.println("结构改变");
        ...
        blMonitorList.addComponentListener(new ComponentAdapter() {
            public void componentResized(ComponentEvent e) {
                int w = blMonitorList.getSize().width;
                if (w < 1) {
        ...
        blMonitorList.getTreeTable().getTree().addTreeExpansionListener(new
        ...
        this.blMonitorList.getTreeTable().addRowSelectedListener(new
            RowSelectedListener() {
        ...
        this.blMonitorList.addDataChangedListener(new DataChangedListener() {
            public void processDataChanged() {
//                refreshGraphic();
            }

        });
        JScrollPane bScrollPane = blMonitorList.getTreeTable().getScrollPane();
...
//        filterTitle.setText("Filter By:");
        ...
        filterLbl.setText(VwBLMonitorFilter.FILTER_CONDITION[0]);
        filterLbl.setToolTipText("Filter By:" +
                                 VwBLMonitorFilter.FILTER_CONDITION[0]);
     ...
     tdArea.getButtonPanel().addButton(blMonitorList.btnWorker);
        //detail button
        ...
        addWBSBtn.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                //执行添加WBS的动作
                blMonitorList.actionPerformedAddWBS(e);
                refreshGraphic();
                actionPerformedSave();
            }
        });
...
//        addActivityBtn = blPlanList.getButtonPanel().addAddButton(new ActionListener() {
        ...
        addActivityBtn.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                blMonitorList.actionPerformedAddActivity(e);
                refreshGraphic();
                actionPerformedSave();
            }
        });
...
//                popWindowForNextVesion();
                copyNode = blMonitorList.getSelectedNode();
                pasteTimes = 1;
                if (copyNode != null) {
                    cutFlag = true;
                    copyNodePath = blMonitorList.getTreeTable().
                                   getSelectionPath();
        ...
        copyBtn.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                ...
                pasteTimes = 1;
                cutFlag = false;
                copyNodePath = blMonitorList.getTreeTable().getSelectionPath();
                if (dto.isActivity()) {
                    copyOnlyActivity = true;
                } else {
                    copyOnlyActivity = false;
        ...
        pasteBtn.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                //执行粘贴操作
                ITreeNode selectedNode = blMonitorList.getSelectedNode();
                String isCut = "false";
                if (cutFlag) {
                    //执行删除动作并清空标志位
                    //blMonitorList.actionPerformedDelete(copyNode);
                    cutFlag = false;
                    isCut = "true";
                }
...
//                      pasteBtn.setEnabled(false);
                      ITreeNode tempNode = blMonitorList.copyNode((DtoWbsTreeNode)copyNode,(DtoWbsTreeNode)selectedNode,isCut);
                     //如果是cut，则删除copyNode
                      if("true".equals(isCut)){
                         blMonitorList.getTreeTable().deleteRow(copyNode);
                     }
                     copyNode = null;
...
//                        selectedNode);
//                    blMonitorList.getTreeTable().addRow(selectedNode, tempNode);
//                    saveBtn.setEnabled(true);
//                }
//                pasteTimes++;
//                blMonitorList.fireProcessDataChange();
//                refreshGraphic();
            }
        });
       ...
       rightBtn.setToolTipText("right");
        ...
        ajustTimingBtn.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                if (blMonitorList.getTreeTable().getSelectedNode() == null) {
                    comMSG.dispErrorDialog("Please select an item!");
                    return;
                }
                VwAdjustTiming adjust = new VwAdjustTiming(blMonitorList.
                    getTreeTable());
        ...
        filterBtn.setToolTipText("Monitoring Filter");
        filterBtn.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                //弹出过滤条件选择窗口
                VwBLMonitorFilter filter = new VwBLMonitorFilter(blMonitorList.
                    getTreeTable(), blMonitorList, filterLbl);
                ...
                if (resultStatus == Constant.OK) {
                    ...
                    if (filterLbl.getText().equals(VwBLPlanFilter.
                        FILTER_CONDITION[0])) {
                        ITreeNode node = (ITreeNode) blMonitorList.getModel().
                                         getRoot();
                        if (node != null &&
                            ((DtoWbsActivity) node.getDataBean()).isReadonly()) {
        ...
        TableColumnChooseDisplay chooseDisplay =
            new TableColumnChooseDisplay(blMonitorList.getTreeTable(),
                                         blMonitorList);
        ...
        computeBtn.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                blMonitorList.actionPerformedCompute(e);
                refreshGraphic();
            }
        });
        ...
        exportBtn = new VWJButtonExcel(new IExcelParameter() {
            public String getUrlAddress() {
                ...
                DtoWbsTreeNode root = (DtoWbsTreeNode) blMonitorList.getModel().
                                      getRoot();
...
//        tdArea.getButtonPanel().addButton(exportBtn);
        ...
        ProcessVariant.addDataListener("monitortree", this);
    }
...
//    }
    ...
    private void setButtonStatus(ITreeNode node) {
        ...
        if (node.getParent() == null) {
            ...
            if (dataBean.isReadonly()) {
                ...
                if (copyNode != null && copyNodePath != null) {
                    TreePath selectedPath = blMonitorList.getTreeTable().
                                            getSelectionPath();
        ...
        if (filterLbl.getText().equals(VwBLMonitorFilter.FILTER_CONDITION[0])) {
            if (dataBean.isReadonly()) {
                ajustTimingBtn.setEnabled(false);
            } else {
                ajustTimingBtn.setEnabled(true);
...
/////// 段2，设置参数：获取传入参数
    public void setParameter(Parameter param) {
        ...
        if (tempFlag != null) {
            templateFlag = true;
            blMonitorList.setTemplateFlag(templateFlag);
...
//最外层的workArea不需要刷新自己
    public void refreshWorkArea() {
        ...
        if (this.refreshFlag) {
            ...
            blMonitorList.setParameter(param);
            blMonitorList.refreshWorkArea();
            leftTree = blMonitorList.getTreeTable().getTree();
    ...
    //默认过滤
    private void defaultFilterHandler() {
        WorkCalendar wc = WrokCalendarFactory.clientCreate();
        Calendar cal = Calendar.getInstance();
        DtoWbsTreeNode root = (DtoWbsTreeNode) blMonitorList.getModel().getRoot();
        DtoWbsActivity dto = (DtoWbsActivity) root.getDataBean();
        ...
        filterDateScope(root, sDate, eDate);
        blMonitorList.getModel().setRoot(root);
        filterLbl.setText(VwBLMonitorFilter.FILTER_CONDITION[4]);
        filterLbl.setToolTipText("Filter By:" +
                                 VwBLMonitorFilter.FILTER_CONDITION[4] + ":" +
                                 comDate.dateToString(sDate) + "~~" +
                                 comDate.dateToString(eDate));
        ajustTimingBtn.setEnabled(false);

    }

    private void filterDateScope(DtoWbsTreeNode node, Date start, Date finish) {
        DtoWbsActivity wbs = (DtoWbsActivity) node.getDataBean();
       if (!node.isLeaf()) {
           List childs = node.children();
           for (int i = childs.size(); i > 0; i--) {
               filterDateScope((DtoWbsTreeNode) childs.get(i - 1), start, finish);
           }
       }
...
//        this.pasteBtn.setEnabled(false);
    ...
    public void actionPerformedSave() {
        ReturnInfo ri = blMonitorList.actionPerformedSave();
        if (!ri.isError()) {
            saveBtn.setEnabled(false);
            detailBtn.setEnabled(true);
//                    blPlanList.resetUI();
            //保存以后设置所有的节点的属性均为正常
            setNodeNomrmal((ITreeNode) blMonitorList.getModel().getRoot());
            blMonitorList.fireProcessDataChange();
        } else {
            //报错了
    ...
    public void actionPerformedDel(ActionEvent e) {
        blMonitorList.actionPerformedDelete(e);
    }

    public void actionPerformedDown(ActionEvent e) {
        ITreeNode node = blMonitorList.getSelectedNode();
        DtoWbsActivity dataBean = (DtoWbsActivity) node.getDataBean();
        if (dataBean.isWbs()) {
            blMonitorList.actionPerformedDownForWBS(e);
        } else if (dataBean.isActivity()) {
            blMonitorList.actionPerformedDownForActivity(e);
        } else {
            return;
        }
    ...
    public void actionPerformedUp(ActionEvent e) {
        ...
        DtoWbsActivity dataBean = (DtoWbsActivity) node.getDataBean();
        if (dataBean.isWbs()) {
            blMonitorList.actionPerformedUpForWBS(e);
        } else if (dataBean.isActivity()) {
            blMonitorList.actionPerformedUpForActivity(e);
        } else {
            return;
        }
    ...
    public void actionPerformedLeft(ActionEvent e) {
        ...
        DtoWbsActivity dataBean = (DtoWbsActivity) node.getDataBean();
        if (dataBean.isWbs()) {
            blMonitorList.actionPerformedLeftForWBS(e);
        } else if (dataBean.isActivity()) {
            //目前情况下此句不会执行
            blMonitorList.actionPerformedLeftForActivity(e);
        } else {
            return;
        }
        this.setButtonStatus(node);
...
//        blPlanList.actionPerformedRight(e);
        DtoWbsActivity dataBean = (DtoWbsActivity) node.getDataBean();
        if (dataBean.isWbs()) {
            blMonitorList.actionPerformedRightForWBS(e);
        } else if (dataBean.isActivity()) {
            blMonitorList.actionPerformedRightForActivity(e);
        } else {
            return;
        }
    ...
    public void actionPerformedExpand(ActionEvent e) {
        blMonitorList.actionPerformedExpand(e);
    }
...
//        param.put(DtoKEY.DTO, dataBean);
            ...
            if (dataBean.isWbs()) {
                param.put("callerType", "Monitoring");
                param.put("upTreeTable", blMonitorList);
...
//            }
    ...
    public void popDetailWindow() {
        ITreeNode node = blMonitorList.getTreeTable().getSelectedNode();
        if (node != null) {
            ...
            if (nodeData.isWbs()) {
                ...
                param.put("listPanel", this.blMonitorList);
    ...
    protected ITreeNode setNodeInsertProp(ITreeNode node, ITreeNode parentNode) {
        ...
        Long wbsRid = blMonitorList.getWbsRid(dataBean);
...
//返回Save Button的状态，如果是可以保存的就会提示用户保存数据
    ...
    public void pasteUpdate(ITreeNode parentNode,ITreeNode newNode){
        ...
        blMonitorList.getTreeTable().addRow(parentNode, newNode);
...
protected Container getMyParentWindow() {
    ...
    public static void main(String[] args) {
        VwBLMonitoring workArea = new VwBLMonitoring();
        Parameter param = new Parameter();
