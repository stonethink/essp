...
public class GanttProject extends GanttProjectBase implements ActionListener,
    ...
    private TaskManager myTaskManager;//原为final
...
//
    ...
    //返回可用的Button的List
    public List getGanttChartBtnList(){
        List btnList=new ArrayList();
        btnList.add(bPrev);
        btnList.add(bScrollCenter);
        btnList.add(bNext);
        btnList.add(bZoomIn);
        btnList.add(bZoomOut);
        return btnList;
    }
    ...
    public GanttProject(boolean isOnlyViewer, boolean isApplet) {
        ...
        Object [][]configs = new Object[][] { {"Name", "name",
                  "Editable", null}, {"Code", "code",
                  "Editable", new JTextField(), Boolean.FALSE},
        };
...
//            tips.toFront(); // somehow assure, that the TipsDialog is the top
        ...
        ChartGridImpl gridImpl=(ChartGridImpl)this.getArea().getChartModel().getChartGrid();
        gridImpl.setProject(this);
    }
...
//                                    "/docs/help_"+sl+"/", "eclipse");
    ...
    private void openDocument(Document document) throws IOException {
        if (document.getDescription().endsWith(".xml")
            ...
            + document.getDescription();
            getUIFacade().showErrorDialog(errorMessage);
            return;
        }
//        Chart[] charts = Mediator.getPluginManager().getCharts();
//        for (int i = 0; i < charts.length; i++) {
...
//        }
    ...
    public void openStartupDocument(String path) {
        if (path != null) {
            Document document = getDocumentManager().getDocument(path);
    ...
    public static void main(String[] arg) {
        ...
        if (false==cmdlineApplication.export(arg)) {
    ...
    //自己加的清空TaskManager的方法
    public void newEmptyTaskManager(){
        class TaskManagerConfigImpl implements TaskManagerConfig {
            public Color getDefaultColor() {
                return myUIConfiguration.getTaskColor();
            }
        ...
        TaskManagerConfig config=new TaskManagerConfigImpl();
        myTaskManager = TaskManager.Access.newInstance(
                new TaskContainmentHierarchyFacade.Factory() {
                    public TaskContainmentHierarchyFacade createFacede() {
                        return GanttProject.this.getTaskContainment();
...
//                    sIcons = sIcons + GanttOptions.DOWN;
    ...
    public void repaint2() {
        ...
        getResourcePanel().getResourceTreeTable().setRowHeight(18);
