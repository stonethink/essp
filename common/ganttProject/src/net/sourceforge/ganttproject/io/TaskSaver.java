...
class TaskSaver extends SaverBase {
    void save(IGanttProject project, TransformerHandler handler, Color defaultColor, CustomColumnsStorage customColumnsStorage) throws SAXException, IOException {
        ...
        writeTaskProperties(handler, customColumnsStorage);
        ...
        for (int i=0; i<tasks.length; i++) {
            writeTask(handler, (GanttTask) tasks[i], customColumnsStorage);
        }
...
//            fout.write("\n" + space2 + "</notes>\n");
        ...
        for (Iterator/*<CustomColumn>*/ it = customColumns.getCustomColums().iterator(); it.hasNext();) {
            ...
            if (GregorianCalendar.class.isAssignableFrom(nextColumn.getType())) {
                value = DateParser.getIsoDate(((GanttCalendar)value).getTime());
            }
            addAttribute("taskproperty-id", idc, attrs);
            addAttribute("value", String.valueOf(value), attrs);
    ...
    private void writeTaskProperties(TransformerHandler handler, CustomColumnsStorage customCol) throws SAXException {
        ...
        while (it.hasNext()) {
            ...
            writeTaskProperty(handler, idcStr, cc.getName(), "custom", valueType, String.valueOf(defVal));
        }
    }
