...
//    private boolean isFinishFixed;
    ...
    public GanttTaskPropertiesBean(GanttTask[] selectedTasks, TaskManager taskManager, HumanResourceManager humanManager, RoleManager roleManager) {
        this.onlyOneTask = false;
        if (selectedTasks.length == 1)
            this.onlyOneTask = true;
        this.selectedTasks = selectedTasks;
        setInitialValues(selectedTasks[0]);
        myHumanResourceManager = humanManager;
...
//		setTree(tree);
    ...
    private JXDatePicker createDatePicker(ActionListener listener) {
        ...
        contentPanel.add(new JXDatePicker());
        SpringUtilities.makeCompactGrid(contentPanel, 1, 2, 0, 0, 0, 0);
...
//        this.taskIsFinishFixed = task.isFinishFixed();
	...
	private JCheckBox constructCheckBox () {
		...
		if (canBeProjectTask) {
			...
			result = new JCheckBox(language.getText("meetingPoint"));
			mileStoneCheckBox1 = result;
		}
		else {
			result = null;
		}
