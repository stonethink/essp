/*
 * $Id: JForm.java,v 1.1 2006/10/10 03:51:01 mingxingzhang Exp $
 *
...
import org.jdesktop.swing.data.TableModelExtAdapter;
 ...
 * </ul>
 * 
 * Changed (JW): the Form can handle non-visual data fields - will not 
 * create a component/binding if the  metaData customProperty 
 * DataConstants.NON_VISUAL_FIELD is set to Boolean.TRUE.
 * 
 ...
 * column in a DefaultTableModelExt (and in the future, a RowSet):
 * <pre><code>
...
public class JForm extends JPanel {
     ...
     * @see MetaData
     * @param tabularData DefaultTableModelExt being bound to the form
     ...
     * @return Binding instance created from bind operation
         * @throws BindException if there were errors when binding to the data model
     */
    public Binding bind(DefaultTableModelExt tabularData, String columnName) throws BindException {
    ...
    public Binding[] bind(DataModel model) throws BindException {
        ...
        for(int i = 0; i < fieldNames.length; i++) {
            Binding binding = bind(model, fieldNames[i]);
            if (binding != null) {
     ...
     * @return Binding instance created from bind operation. Will be null for 
     *   non-visual fields.
     * @throws BindException if there were errors when binding to the data model
     */
    public Binding bind(DataModel model, String fieldName) throws BindException {
        if (getFormFactory().isNonVisual(model.getMetaData(fieldName))) return null;
    ...
    /**
     * Added to ease subclassing without actually 
     * exposing the private bindings-list. 
     * 
     * @param binding
     */
    ...
    private DataModel getDataModelWrapper(Object model) throws BindException {
        ...
        if (wrapper == null) {
            if (model instanceof DefaultTableModelExt) {
                wrapper = new TableModelExtAdapter((DefaultTableModelExt)model);
            } else {
                try {
