...
<html>
  <head>
	...
	function onAddData(){      
	   ...
	   var url='<%=contextPath%>/hrbase/bd/BDAdd.jsp';
	   var windowTitle="";
    ...
    if(confirm("<bean:message bundle="application" key="global.confirm.delete"/>")) {
    	window.location="<%=contextPath%>/hrb/bd/deleteBdCode.do?code="+currentRow.selfproperty;
    }
   ...
   //当进入此页时，自动选中第一行，并国际化表头
	...
	try{
        ...
        var str0="<bean:message bundle="hrbase" key="hrbase.bd.bdcode"/>";
		var str1="<bean:message bundle="hrbase" key="hrbase.bd.bdname"/>";
		var str2="<bean:message bundle="hrbase" key="hrbase.enable"/>";
        var str3="<bean:message bundle="hrbase" key="hrbase.bd.isAchieveBelong"/>";
		var thead_length=table.tHead.rows.length;
		...
		cells[3].innerHTML=cells[3].innerHTML.replace("achieveBelong",str3);
		 var firstRow=table.rows[thead_length];
	       disableBtn();
		   rowOnClick(firstRow);
...
<%
  ...
  %>
   <ec:table 
       ...
       action="${pageContext.request.contextPath}/hrb/bd/listAllBdCode.do"  
       ...
       >
        <ec:row onclick="rowOnClick(this);" selfproperty="${bCode.bdCode}">
             ...
             <ec:column 
                 ...
                 filterable="false" 
                 sortable="false" 
                 cell="server.essp.hrbase.bd.action.EcAreaCodeStatus"
                />            
             <ec:column 
                 alias="checkbox2"
                 title="achieveBelong" 
                 ...
                 filterable="false" 
                 sortable="false" 
                 cell="server.essp.hrbase.bd.action.EcBdIsAchieveBelong"
              />    
        </ec:row>        
     </ec:table>
     <SCRIPT language="JavaScript" type="Text/JavaScript">
