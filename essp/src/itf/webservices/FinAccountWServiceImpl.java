...
//            printMap(accountFields);
            ...
            String sSql = "insert into PROJBASE (PROJID,PROJNAME,NICKNAME,MANAGER,LEADER,COSTDEPT,TCSIGNER,"
                          ...
                          +"ICONT_TEL,ICONT_EMAIL, BL) values(?,?,?,?,?,?,?,?,?,?,"+
            ...
            st.setString(57, projectBase.getBl());
            st.executeUpdate();
            conn.commit();
...
//        }
        ...
        try {
            ...
            st.setString(55, projectBase.getBl());
            st.setString(56, projectBase.getProjId());
            st.executeUpdate();
            conn.commit();
    ...
    public void closeAccount(String acntId, Map accountFields) {
        ...
        PreparedStatement st = null;
         try {
        	 ProjectBase projectBase = new ProjectBase();
             DtoUtil.copyMapToBean(projectBase, accountFields);
             String sSql = "update PROJBASE set CLOSEMARK='Y',ACTUFROM=?,ACTUTO=?,"
                          +"MANMONTH=CONVERT(money,'" + projectBase.getManMonth() +"') where PROJID='" + acntId + "'";
//             System.out.println(sSql);
             st = conn.prepareStatement(sSql);
             st.setString(1, projectBase.getActuFrom());
             st.setString(2, projectBase.getActuTo());
             st.executeUpdate();
             conn.commit();
	...
	public void reopenOrCloseAccount(String acntId, String status) {
        ...
        Statement st = null;
         try {
             ...
             String sSql = "update PROJBASE set CLOSEMARK='" + status + "' where PROJID='" + acntId + "'";
//             System.out.println(sSql);
             ...
             try {
                 log.error("error while reopening account from web services!",ex);
                 conn.rollback();
                 throw new BusinessException("Reopen Finance ERROR", ex);
             } catch (Exception ex1) {
                 log.error(ex1);
                 throw new BusinessException("Reopen Finance Rollback ERROR", ex1);
             }
         } finally {
             if (st != null) {
                 try {
	...
	public void saveOrUpdateEmp(Map empInfo) {
		initDbCon();
        PreparedStatement st = null;
        PreparedStatement stSearch = null;
        PreparedStatement stUpdate = null;
        ResultSet rs = null;
        String sql = "select * from EMPLOYEE where lower(EMPID)=lower(?)";
        String sSql = "insert into EMPLOYEE (EMPID, EMPNAME, DEPTID, IN_JOB_DATE, QUIT_DATE) "
        	+ " values(?, ?, ?, ?, ?)";
        String upSql = "update EMPLOYEE set EMPNAME=?, DEPTID=?, IN_JOB_DATE=?, QUIT_DATE=? where lower(EMPID)=lower(?)";
        try {
        	EmployeeBase employeeBase = new EmployeeBase();
        	DtoUtil.copyMapToBean(employeeBase, empInfo);
        	stSearch = conn.prepareStatement(sql);
        	stSearch.setString(1, employeeBase.getEmpId());
        	rs = stSearch.executeQuery();
        	if(rs != null && rs.next()) {
        		stUpdate = conn.prepareStatement(upSql);
        		stUpdate.setString(1, employeeBase.getEmpName());
        		stUpdate.setString(2, employeeBase.getDeptId());
        		stUpdate.setString(3, employeeBase.getInJobDate());
        		stUpdate.setString(4, employeeBase.getQuitDate());
        		stUpdate.setString(5, employeeBase.getEmpId());
        		stUpdate.executeUpdate();
        	} else {
        		st = conn.prepareStatement(sSql);
    			st.setString(1, employeeBase.getEmpId());
    			st.setString(2, employeeBase.getEmpName());
    			st.setString(3, employeeBase.getDeptId());
    			st.setString(4, employeeBase.getInJobDate());
    			st.setString(5, employeeBase.getQuitDate());
    			st.executeUpdate();
        	}
			conn.commit();
		} catch (Exception e) {
			try {
                log.error("error while Synchronise employee from web services!",e);
                conn.rollback();
                throw new BusinessException("Synchronise Finance Employee ERROR", e);
            } catch (Exception ex) {
                ex.printStackTrace();
                throw new BusinessException("Synchronise Finance Employee Rollback ERROR", ex);
            }
		} finally {
            try {
				if (st != null) {
