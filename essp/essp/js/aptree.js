    ...
    if(linkString.length>=4) {
        ...
        if(beignstr.charAt(0)=='U' &&
           ...
           if(beignstr.charAt(0)=='S' &&
              ...
              //eval(perStr);
              /*
              modi by Xuxi.Chen,??????Script??????????pms/pms.jsp????????????????????
              ??????????????????????????????????????????????????????????????
              ??????????????????????????
              */
              // add Math.random() ?????Local??????
              if(!parent.main.Account){window.open("TSAccount.jsp?" + Math.random(), "main");}
              setTimeout('waitExec(\''+bodystr+'\')',2000);
