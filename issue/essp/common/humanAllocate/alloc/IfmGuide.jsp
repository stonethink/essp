...
function insert(parent,brother){
  ...
  var tdloginid = document.createElement('<td width="38%" class="oracelltext" id="tdloginid'+num+'"></td>');
  var tdname=document.createElement('<td width="36%" class="oracelltext" id="tdname'+num+'"></td>');
  var tdcheck = document.createElement('<td width="18%" id="tdcheck'+num+'"></td>');
  p.insertBefore(tr,b);
...
function moveout() {
    ...
    for(i=1;i<iLen;i++) {
      ...
      if(document.alloc_tool_guide.chRes[i].checked) {
...
function resetIt() {
    var ifreset = confirm("Are you sure to reset?");
	if(!(ifreset)) {
	    return false
	}
    //进入页面时记录的条数
...
function showLog(count){
    if(count==0) {
      window.parent.itemCount.innerHTML = "&nbsp;<span class='np'>2</span>&nbsp;There is <strong>No</strong> item in account now";
    } else if(count==1){
      window.parent.itemCount.innerHTML = "&nbsp;<span class='np'>2</span>&nbsp;There is <strong>1</strong> item in account now";
    } else {
      window.parent.itemCount.innerHTML = "&nbsp;<span class='np'>2</span>&nbsp;There are <strong>"+count+"</strong> items in account now";
    }
}
...
function onBodyLoad(){
  try{
    ...
    for(var i = 0 ;i < oldDataArray.length;i ++){
      if(oldDataArray[i]!=""){
        add(oldDataArray[i],oldDataArray[i],true);
        sum ++;
      }
    }
    for(var j = 0 ;j < laborArray.length; j ++){
      ...
      if(!existed && laborArray[j]!=""){
        add(laborArray[j],laborArray[j],false);
        sum ++;
      }
    }
