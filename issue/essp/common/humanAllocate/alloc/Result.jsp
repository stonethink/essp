...
function checkall() {
  //如果当前点击选中所有的items
  if(document.alloc_tool_result.chSelectall.checked) {
      ...
      if(check_count==1) {
        tdItem.innerText = "1 item selected";
      } else {
        tdItem.innerText = check_count + " items selected";
      }
    ...
    tdItem.innerText = "No item selected";
    window.result.frm21.bot_left_frm.count_check.value = 0;
  }
}

<%-- 添加查询结果，只添加loginid,name,code和post  --%>
function add(prm_loginid,prm_name,prm_code,prm_sex) {
	...
	for(i=1;i<=iAll;i++) {
        ...
        //分别添加左边和右边
    window.result.frm21.add(prm_loginid,prm_name);
...
<form name="alloc_tool_result">
  <table class="tablecenter" CELLPADDING="0" CELLSPACING="0" width="98%"  align="center">
    <tr>
      ...
      <td id="td1" width="100" valign="bottom" background="../images/now_back.jpg" class="list_desc">
        <div align="center" onclick=change(1); style="CURSOR: hand">Allocation Result</div></td>
  ...
  <table width="100%" border="0" cellpadding="1" cellspacing="1">
    <tr>
      <td width="33%" align="center" class="howmany" id="tdItem">No item selected</td>
      <logic:present parameter="oldValue">
        <td width="52%" class="howmany"><input name="in" type="button" class="button" style="width:60" onclick="return finish()" value="Finish"></td>
      </logic:present>
      <logic:notPresent parameter="oldValue">
        <td width="52%" class="howmany"><input name="in" type="button" class="button" style="width:60" onclick="return exportit()" value="《 IN"></td>
      </logic:notPresent>
      <td width="16%" ><input name="button" type="button" class="button" style="width:60" onClick="return deleteit()" value="Delete"></td>
    </tr>
  </table>
  ...
  <div id="Layer1" class="divtype" style="position:absolute; z-index:0; left: 7px; visibility: visible; top: 26px; height:73%; width=98%;">
      <logic:notPresent parameter="oldValue">
       ...
       <logic:present parameter="oldValue">
         <iframe name=result
           src="IfmResult.jsp?oldValue=<bean:write name="oldValue"/>"
           frameBorder=0 width="98%"
           height="99%"
           scrolling="no"
           align="left">
